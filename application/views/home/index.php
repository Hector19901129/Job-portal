...
<div class="dashboard-window clearfix" style="background: #62acec; border-left: 5px solid #5798d1;">
	...
	<div class="d-w-text">
		 <span class="d-w-num"><?php echo number_format($stats->total_jobs) ?></span><br /><?php echo lang("ctn_476") ?>
	</div>
</div>
</div>
...
<div class="dashboard-window clearfix" style="background: #5cb85c; border-left: 5px solid #4f9f4f;">
	...
	<div class="d-w-text">
		 <span class="d-w-num"><?php echo number_format($stats->total_assigned_jobs) ?></span><br /><?php echo lang("ctn_477") ?>
	</div>
</div>
</div>
...
<div class="dashboard-window clearfix" style="background: #f0ad4e; border-left: 5px solid #d89b45;">
	...
	<div class="d-w-text">
		 <span class="d-w-num"><?php echo number_format($stats->jobs_today) ?></span><br /><?php echo lang("ctn_478") ?>
	</div>
</div>
</div>
...
<tbody>
<?php foreach($assigned_jobs->result() as $r) : ?>
    <?php
    if($r->status == 0) {
        $status = lang("ctn_465");
    } elseif($r->status == 1) {
    ...
    if(isset($r->client_username)) {
        $user = $this->common->get_user_display(array("username" => $r->client_username, "avatar" => $r->client_avatar, "online_timestamp" => $r->client_online_timestamp, "user" => false));
    } else {
        $user = '<div class="user-box-avatar"><img src="'.base_url().$this->settings->info->upload_path_relative.'/guest.png'.'" data-toggle="tooltip" data-placement="bottom" title="'.$r->guest_email.'"></div>';
            ...
            ?>
<tr><td><a href="<?php echo site_url("jobs/view/" . $r->ID) ?>"><?php echo $r->title ?></a></td><td><?php echo $prioritys[$r->priority] ?></td><td><?php echo $status ?></td><td><?php echo $user ?></td><td><?php echo date($this->settings->info->date_format,$r->last_reply_timestamp) ?></td></tr>
<?php endforeach; ?>
</tbody>
</table>
</div>

<a href="<?php echo site_url("jobs/assigned") ?>" class="btn btn-primary btn-sm form-control"><?php echo lang("ctn_482") ?></a>
</div>
</div>
...
<table class="table">
<?php foreach($your_jobs->result() as $r) : ?>
    <?php
    if(isset($r->client_username)) {
        $user = $this->common->get_user_display(array("username" => $r->client_username, "avatar" => $r->client_avatar, "online_timestamp" => $r->client_online_timestamp, "user" => false));
    } else {
...
</td><td><p class="task-blob-title"><a href="<?php echo site_url("jobs/view/" . $r->ID) ?>"><?php echo $r->title ?></a></p>
...
<a href="<?php echo site_url("jobs/your") ?>" class="align-center btn btn-primary btn-sm"><?php echo lang("ctn_461") ?></a>
</div>
</div>
</div>
        ...
        datasets: [
            {
                ...
                pointRadius: 1,
                pointHitRadius: 10,
                data: [<?php foreach($close_jobs as $i) : ?>
                    <?php echo $i['count'] ?>,
                <?php endforeach; ?>],
                spanGaps: false,
            },
            {
                ...
                pointRadius: 1,
                pointHitRadius: 10,
                data: [<?php foreach($open_jobs as $i) : ?>
                    <?php echo $i['count'] ?>,
                <?php endforeach; ?>],
                spanGaps: false,
            },
