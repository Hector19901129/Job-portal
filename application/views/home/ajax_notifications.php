...
<div class="notification-box-bit animation-fade clearfix <?php if(!$r->status) : ?>active-noti<?php endif; ?>">
  <div class="notification-icon-bit">
	...
	<?php else : ?>
		<img src="<?php echo base_url() ?>/<?php echo $this->settings->info->upload_path_relative ?>/default.png" class="user-icon">
	<?php endif; ?>
  </div>
  <div class="notification-text-bit click" onclick="load_notification_url(<?php echo $r->ID ?>)">
    <?php if(isset($r->username)) : ?><a href="<?php echo site_url("profile/" . $r->username) ?>"><?php echo $r->username ?></a><?php endif; ?> <?php echo $r->message ?>
