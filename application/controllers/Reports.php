...
class Reports extends CI_Controller
{
	...
	public function index($type=0)
	{
		...
		if($type == 0) {
			...
			{
				$count = $this->jobs_model->get_jobs_for_day($date);
				$results[] = array(
					"date" => $date,
					"count" => $count
					);
		...
		if($type == 1) {
			foreach($dates as $date)
			{
				$count = $this->jobs_model->get_jobs_for_day_closed($date);
				$results2[] = array(
					"date" => $date,
					"count" => $count
					);
	...
	public function rating_page()
	{
		...
		$this->datatables->ordering(
			...
			$this->datatables->set_total_rows(
				$this->jobs_model
					->get_jobs_rating_total()
			);
			$jobs = $this->jobs_model->get_rating_jobs($this->datatables);
		...
		foreach($jobs->result() as $r) {
			...
			for($i=1;$i<=5;$i++) {
			    if($i > $r->rating) {
			      $rating.='<span class="glyphicon glyphicon-star-empty click" id="job'. $i .'"></span>';
			    }else {
			      $rating.='<span class="glyphicon glyphicon-star click" id="job'. $i .'"></span>';
			    }
  			}
	...
	public function user_page()
	{
		...
		$this->datatables->ordering(
			...
			$jobs = $this->jobs_model->get_user_ratings($this->datatables);
