...
class Jobs_Model extends CI_Model
{
	...
	//newly added
	public function updateFieldOrder($field_order){
		$this->db->where("ID", 1)->update("site_settings", $field_order);
	}
	//newly added
	...
	public function get_client_jobs($userid, $datatable)
	{
		...
		return $this->db
			...
			->where("j1.fieldid", 9)
			->where("j2.fieldid", 10)
			->where("jobs.visible", 1)
			->select("jobs.ID, jobs.title, j1.value nm2, j2.value nm, jobs.userid, jobs.assignedid,
				jobs.timestamp, jobs.categoryid, jobs.status,
				jobs.priority, jobs.last_reply_timestamp,
				jobs.last_reply_userid, jobs.message_id_hash,
				jobs.guest_email,
			...
			->join("users as u3", "u3.ID = jobs.last_reply_userid", "left outer")
			->join("job_categories", "job_categories.ID = jobs.categoryid")
			->join("job_user_custom_fields as j1", "j1.jobid = jobs.ID", "inner")
			->join("job_user_custom_fields as j2", "j2.jobid = jobs.ID", "inner")
			->limit($datatable->length, $datatable->start)
			->get("jobs");
	}
