...
class Home extends CI_Controller 
{
	...
	public function index()
	{
		if(!$this->common->has_permissions(array("admin", "admin_settings",
		 ...
		 "job_manager", "job_worker", "knowledge_manager"), $this->user)) {
		...
		for($i=11;$i>=0;$i--) {
			...
			$count = $this->jobs_model->get_jobs_for_month($new_month, $current_year);
			$months[] = array(
		...
		for($i=11;$i>=0;$i--) {
			...
			// Get month name using mktime
			$timestamp = mktime(0,0,0,$new_month,1,$current_year);
			$count = $this->jobs_model->get_jobs_for_month_closed($new_month, $current_year);
			$months[] = array(
				"date" => date("F", $timestamp),
				"count" => $count
				);
		...
		$your_jobs = $this->jobs_model->get_jobs_your_limit($this->user->info->ID, 0, 5);
		$assigned_jobs = $this->jobs_model->get_jobs_assigned_limit($this->user->info->ID, 0, 5);
	...
	private function get_fresh_results($stats) 
	{
		...
		$data->total_jobs = $this->jobs_model->get_jobs_total_no_view(0);
		$data->total_assigned_jobs = $this->jobs_model->get_jobs_assigned_total_no_view($this->user->info->ID, 0);
		$data->jobs_today = $this->jobs_model->get_jobs_today(date("d-n-Y"));
	...
	public function read_all_noti($hash) 
	{
		...
		$this->session->set_flashdata("globalmsg", "All notifications have been marked as read!");
