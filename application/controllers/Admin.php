...
class Admin extends CI_Controller 
{
	...
	public function job_settings() 
	{
		...
		$this->template->loadData("activeLink", 
			array("admin" => array("job_settings" => 1)));

		$categories = $this->jobs_model->get_categories();
		$this->template->loadContent("admin/job_settings.php", array(
			"categories" => $categories
			)
		);
	}

	public function job_settings_pro() 
	{
		if(!$this->common->has_permissions(array("admin", "admin_settings",), 
			$this->user)) {
			$this->template->error(lang("error_2"));
		}
		$enable_job_uploads = intval($this->input->post("enable_job_uploads"));
		$enable_job_guests = intval($this->input->post("enable_job_guests"));
		$enable_job_edit = intval($this->input->post("enable_job_edit"));
		...
		$job_title = $this->common->nohtml($this->input->post("job_title"));
		$protocol_ssl = intval($this->input->post("protocol_ssl"));

		$default_category = intval($this->input->post("catid"));

		$imap_job_string = $this->common->nohtml($this->input->post("imap_job_string"));
		$imap_reply_string = $this->common->nohtml($this->input->post("imap_reply_string"));

		$job_rating = intval($this->input->post("job_rating"));

		$captcha_job = intval($this->input->post("captcha_job"));
		...
		$this->admin_model->updateSettings(
			array(
				"enable_job_uploads" => $enable_job_uploads,
				"enable_job_guests" => $enable_job_guests,
				"enable_job_edit" => $enable_job_edit,
